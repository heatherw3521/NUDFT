function [relresid, relerr, times, condA] = iterative_test2(solvlist, signal,N,dataratio,x, wts, condflag, opts)
...
if strcmp(signal, 'randn')          % choose signal: c0 = true Fourier coeffs
  c0 = randn(N,nr) + 1i*randn(N,nr);     % flat spectrum
elseif strcmp(signal, 'decay')
  c0 = exp(-10.0*abs(freqinds/N)) .* (randn(N,nr) + 1i*randn(N,nr));  % exp decay
elseif strcmp(signal, 'image')     % the F coeffs define this func, not fun(x) (only 1 RHS!)
